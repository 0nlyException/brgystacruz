			...
			if(mysqli_num_rows($query_run) > 0) {
				foreach ($query_run as $event) {
					...
					<div class="announcement-container">
						<div>
							<h2 style="color: black;font-weight: 700"><?= $event['title']; ?></h2>
							<p style="color: black;font-weight: 600"><?= $event['description']; ?></p>
						</div>
						<?php
						...
						if (!empty($event['image'])) {
							...
							<img src="data:image/jpeg;base64,<?= base64_encode($event['image']); ?>" alt="<?= $event['title']; ?>">
							<?php
						} else {
							?>
